...
class User(db.Model):
    ...
    # 为了方便查看对象输出内容,重写repr
    def __repr__(self):
        return "<User:%s,%s,%s,%s,%s>" % (self.id, self.code, self.name, self.password, self.create_time.strftime("%Y-%m-%d-%H"))
...
class Basefile(db.Model):
    ...
    type = db.Column(db.String(10), nullable=False)  # 病历类型
    plan_code = db.Column(db.String(4), nullable=False)  # 方案编码
    field_name = db.Column(db.String(50), nullable=False)  # 字段名
    basefile_name = db.Column(db.String(50), default='')  # 对应档案名
    colid = db.Column(db.Integer, default=0)
    print_value = db.Column(db.String(50), default='')
    default_value = db.Column(db.String(50), default='')
    list_order = db.Column(db.Integer, default=0)
    sort_type = db.Column(db.String(10), default='')
...
class Programme(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    code = db.Column(db.String(10), nullable=False)  # 方案编码
    name = db.Column(db.String(50), nullable=False)  # 方案名称
    type = db.Column(db.String(10), nullable=False)  # 病历类型

    def __repr__(self):
        return '<Programme %r, %r, %r, %r>' % (self.id, self.code, self.name, self.type)
